...
export function LoginForm() {
  ...
  return <section className="rounded-3xl border border-line bg-white p-[clamp(1.4rem,4vw,2.5rem)] shadow-card">
    <div className="mb-6 grid grid-cols-2 gap-1 rounded-xl bg-[#edf3f3] p-1" role="tablist" aria-label="Tipo de usuario">
      {(["Administración", "Colaborador"] as const).map((tab) => <button key={tab} role="tab" aria-selected={role === tab} className={`rounded-lg p-3 font-bold text-muted ${role === tab ? "bg-white text-primary-dark shadow-soft" : ""}`} type="button" onClick={() => setRole(tab)}>{tab}</button>)}
    </div>
    <form className="grid gap-2" onSubmit={submit}>
      <button className="mb-2 min-h-10 rounded-xl border border-primary/20 bg-primary-soft px-3 text-sm font-extrabold text-primary-dark transition hover:border-primary/40 hover:bg-[#d8eeee]" type="button" onClick={() => { setEmail("admin@docli.local"); setPassword("DocliDemo2026!"); setError(""); }}>
      ...
      <Link className="mt-1 text-center text-sm text-muted" href="/contacto">Recuperar contraseña</Link><p className="m-1 text-center text-sm text-muted">¿No tienes una cuenta aún?</p><Link className="inline-flex min-h-11 items-center justify-center rounded-xl bg-primary-soft font-extrabold text-primary-dark no-underline transition hover:bg-[#d8eeee]" href="/registro">Crear una cuenta</Link>
    </form>
