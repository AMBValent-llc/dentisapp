    ...
    <main className="overflow-clip" id="main-content">
      ...
      <section className="relative z-10">
        <div className="relative z-10 mx-auto grid min-h-170 w-full max-w-330 grid-cols-[minmax(0,.86fr)_minmax(500px,1.14fr)] items-center gap-[clamp(2.5rem,7vw,7rem)] px-[clamp(1.25rem,4vw,3.5rem)] py-[clamp(4rem,8vw,7rem)] max-[880px]:grid-cols-1 max-sm:min-h-0 max-sm:pt-14">
          <Reveal className="text-white">
            ...
            <p className="max-w-142.5 text-lg text-white/90">Diseña cómo debe hacerse el trabajo, asigna responsables y sigue cada avance sin perseguir mensajes. Docli reúne procesos, tareas, documentos y resultados en una sola vista.</p>
            ...
            <div className="flex flex-wrap gap-4 text-xs font-semibold text-white/90"><span>✓ Puesta en marcha rápida</span><span>✓ Roles y trazabilidad</span><span>✓ Módulos especializados</span></div>
          </Reveal>
          ...
          <header className="flex h-12 items-center gap-4 border-b border-[#e7eeee] px-3 text-[.65rem]"><b className="flex items-center gap-1.5 text-primary"><Image className="size-6 object-contain" src="/docli-isotipo.png" width={24} height={24} alt="" /> Docli</b><span className="ml-auto flex-1 rounded-lg bg-[#f3f7f7] px-2 py-1 text-[#9aabad]">Buscar procesos, tareas…</span><i className="grid size-6 place-items-center rounded-lg bg-primary not-italic text-white">AT</i></header>
      ...
      <Reveal as="section" className="relative z-10 px-[max(1.25rem,calc((100vw-1180px)/2))] py-[clamp(4rem,8vw,7rem)]" aria-labelledby="features-title">
        ...
        <div className="grid grid-cols-3 gap-4 max-[880px]:mx-auto max-[880px]:max-w-155 max-[880px]:grid-cols-1">
          {[["◎","Procesos repetibles","Define pasos, responsables y criterios para ejecutar siempre con el mismo estándar."],["✓","Trabajo coordinado","Ordena pendientes, fechas y bloqueos sin perder contexto entre canales."],["↗","Decisiones con evidencia","Consulta avances, tiempos y carga del equipo antes de ajustar la operación."]].map((feature, index) => <Reveal as="article" className="flex items-start gap-4 rounded-[18px] border border-line bg-[#fbfdfd] p-5 transition-[opacity,transform,box-shadow,border-color] hover:-translate-y-1 hover:border-primary/35 hover:shadow-soft max-sm:gap-3 max-sm:p-4" delay={index * 90} key={feature[1]}><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary-soft text-primary">{feature[0]}</span><div className="min-w-0"><h3 className="mb-2 text-lg font-bold leading-tight">{feature[1]}</h3><p className="text-muted">{feature[2]}</p></div></Reveal>)}
        </div>
      </Reveal>
      </div>
