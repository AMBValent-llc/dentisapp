...
export default async function PatientsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const visible = await prisma.patient.findMany({ where: { organizationId: organization.id, ...(query ? { OR: [{ fullName: { contains: query, mode: "insensitive" } }, { documentNumber: { contains: query } }] } : {}) }, include: { histories: { select: { createdAt: true }, orderBy: { createdAt: "desc" }, take: 1 } }, orderBy: { fullName: "asc" } });
