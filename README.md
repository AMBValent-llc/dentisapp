...
Prisma genera dos clientes: `generated/prisma` para Node.js y `generated/prisma-cloudflare` para Workers. `cf:build` establece `PRISMA_CLIENT_RUNTIME=cloudflare` y Next.js selecciona el segundo mediante un alias de Webpack; así el compilador WebAssembly se importa como módulo, sin compilar WASM dinámicamente dentro del Worker. El desarrollo y el seed conservan el cliente Node.js. Usa los scripts `cf:*` para Workers; un cliente Node.js puede compilar correctamente y aun así fallar al consultar la base con `Wasm code generation disallowed by embedder`.
